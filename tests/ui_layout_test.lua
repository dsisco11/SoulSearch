...
return function(test, repo_root)
    ...
    test.case('UI layout: baseline panel geometry is exact', function()
        ...
        test.assert_sequence({95}, layout.DIVIDER_XS)
        test.assert_equal(1, layout.RESULTS_LEFT)
        test.assert_equal(94, layout.RESULTS_WIDTH)
        test.assert_equal(97, layout.STATS_LEFT)
        ...
        test.assert_equal(50, layout.RESULT_NAME_WIDTH)
        test.assert_equal(9, layout.RESULT_UNIT_ID_WIDTH)
        test.assert_equal(33, layout.RESULT_PROFESSION_WIDTH)
        test.assert_equal('name', layout.get_result_header_column(0, 0))
        test.assert_equal('name', layout.get_result_header_column(49, 0))
        test.assert_nil(layout.get_result_header_column(50, 0))
        test.assert_equal('profession', layout.get_result_header_column(51, 0))
        test.assert_equal('profession', layout.get_result_header_column(83, 0))
        test.assert_equal('unit_id', layout.get_result_header_column(85, 0))
        test.assert_nil(layout.get_result_header_column(94, 0))
    end)
    ...
    test.case('UI layout: race control extends the filter panel in order', function()
        ...
        test.assert_equal(0, layout.get_frame('unit_scope').t)
        test.assert_equal(1, layout.get_frame('unit_scope_picker').t)
        ...
        test.assert_equal(1, layout.get_frame('add_filter').t)
        test.assert_equal(2, layout.get_frame('add_skill').t)
        test.assert_equal(3, layout.get_frame('add_race').t)
        test.assert_equal(4, layout.get_frame('clear_filters').t)
        test.assert_equal(5, layout.get_frame('presets').t)
