...
return function(test, repo_root)
    ...
    test.case('UI layout: race control extends the filter panel in order', function()
        test.assert_equal(4, layout.get_frame('add_filter').t)
        test.assert_equal(5, layout.get_frame('add_skill').t)
        test.assert_equal(6, layout.get_frame('add_race').t)
        test.assert_equal(7, layout.get_frame('clear_filters').t)
        test.assert_equal(8, layout.get_frame('presets').t)
        test.assert_equal(10, layout.get_frame('filter_list').t)
    end)
